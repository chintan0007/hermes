...
class PolymarketAlphaBot:
    ...
    async def adjust_strategy_from_memory(self):
        ...
        try:
            ...
            loss_rate = (len(recent_trades) - recent_wins) / len(recent_trades)
            # Flat logic to avoid IndentationErrors
            if loss_rate > 0.5:
                self.current_edge_threshold = 0.10
                self.current_multiplier = 0.5
                logger.warning("⚠️ LOSING STREAK: Tightening.")
            else:
                self.current_edge_threshold = 0.05
                self.current_multiplier = 1.0
            ...
            try:
                ...
                if KRONOS_AVAILABLE and self.ml_engine:
                    ...
                    if random.random() > 0.2:
                        ...
                        continue
                    prediction = {"token_id": "0x123", "win_probability": random.uniform(0.52, 0.70), "edge": random.uniform(0.02, 0.15), "price": 0.60}

                if prediction['edge'] < self.current_edge_threshold:
                    await asyncio.sleep(10)
                    continue

                proposed_size_pct = self.risk_manager.calculate_quarter_kelly(self.current_balance, prediction['win_probability'], prediction['edge'])
